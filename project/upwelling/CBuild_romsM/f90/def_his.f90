...
!
      DEFINE : IF (ldef) THEN
        ...
        IF (FoundError(exit_flag, NoError, 183, MyFile)) THEN
...
!
     ...
     &                 NT(ng), DimIDs(11))
        IF (FoundError(exit_flag, NoError, 279, MyFile)) RETURN
        status=def_dim(ng, model, HIS(ng)%ncid, ncname, 'boundary',     &
     &                 4, DimIDs(14))
        IF (FoundError(exit_flag, NoError, 322, MyFile)) RETURN
...
!
     ...
     &                   SetFillVal = .FALSE.)
          IF (FoundError(exit_flag, NoError, 655, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, u3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1013, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, v3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1070, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                     NF_FOUT, nvd4, t3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1233, MyFile)) RETURN
          END IF
        END DO
!
...
!
     ...
     &                   NF_FOUT, nvd4, t3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1282, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd3, t2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1808, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd3, u2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1867, MyFile)) RETURN
        END IF
!
...
!
        ...
        IF (FoundError(exit_flag, NoError, 1947, MyFile)) RETURN
      END IF DEFINE
!
!=======================================================================
...
!
        ...
        IF (FoundError(exit_flag, NoError, 1962, MyFile)) THEN
...
!
     ...
     &                         ncid = HIS(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 1971, MyFile)) RETURN
!
!  Inquire about the variables.
!
        CALL netcdf_inq_var (ng, model, ncname,                         &
     &                       ncid = HIS(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 1977, MyFile)) RETURN
!
