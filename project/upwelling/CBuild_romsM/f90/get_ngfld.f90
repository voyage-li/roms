...
!
     ...
     &                         Fout)
        CASE DEFAULT
          ...
          exit_flag=2
      END SELECT
      IF (FoundError(exit_flag, NoError, 107, MyFile)) RETURN
!
...
!
      ...
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Utility/get_ngfld.F"//", get_ngfld_nf90"
!
      SourceFile=MyFile
!
!-----------------------------------------------------------------------
...
!
     ...
     &                            start = (/Trec/),                     &
     &                            total = (/1/))
            IF (FoundError(exit_flag, NoError, 271, MyFile)) THEN
...
!
     ...
     &                              total = (/Iend-Istr+1,Jrec,1/))
            END IF
            IF (FoundError(exit_flag, NoError, 324, MyFile)) THEN
