...
!
      implicit none
!
      INTERFACE lbc_getatt
        MODULE PROCEDURE lbc_getatt_nf90
      END INTERFACE lbc_getatt
!
      INTERFACE lbc_putatt
        MODULE PROCEDURE lbc_putatt_nf90
      END INTERFACE lbc_putatt
!
      CONTAINS
!
!***********************************************************************
...
!
     ...
     &  "ROMS/Utility/lbc.F"//", lbc_getatt_nf90"
!
!-----------------------------------------------------------------------
...
!
      IF (InpThread) THEN
        ...
        IF (FoundError(status, nf90_noerr, 116,  MyFile)) THEN
...
!
     ...
     &                    TRIM(ncname)
                ...
                END IF
              CASE ('Mix')
                IF (.not.S(ibry,ifield,ng)%mixed) THEN
                  IF (Master) THEN
                    WRITE (stdout,20) B(ibry),                          &
     &                    TRIM(Vname(1,idBvar(ifield))),                &
     &                    TRIM(string(ibry)),                           &
     &                    'S(',ibry,ifield,ng,')%mixed',                &
     &                     S(ibry,ifield,ng)%mixed,                     &
     &                     TRIM(ncname)
                  END IF
                  exit_flag=5
     ...
     &                              TRIM(string(ibry)), TRIM(ncname)
      ...
      END DO
!
  10  FORMAT (/,' LBC_GETATT_NF90 - error while reading global ',       &
     &        'attribute:',2x,a,/,19x,'in restart file:',2x,a,/,        &
     &        19x,'call from:',2x,a,                                    &
     &        /,19x,'Probably global attribute was not found ...',      &
     &        /,19x,'restart file needs to be generated by ROMS ',      &
     &        'version 3.6 or higher',                                  &
     &        /,19x,'Alternatively, you may use  NO_LBC_ATT  at your ', &
     &        'own risk!')
  20  FORMAT (/,' LBC_GETATT_NF90 - inconsistent ',a,' lateral',        &
     &        'boundary condition for variable: ',2x,a,                 &
     &        /,19x,'restart file LBC keyword = ',1x,a,                 &
     &        /,19x,'but assigned structure switch: ',                  &
     &         1x,a,i1,',',i2,',',i1,a,' = ',l1,                        &
     &        /,19x,'check input script LBC keyword for consitency ...',&
     &        /,19x,'restart file:',2x,a)
  30  FORMAT (/,' LBC_GETATT_NF90 - inconsistent ',a,' boundary for ',  &
     &        'variable: ',a,2x,'Keyword = ',a,/,19x,'in input file:',  &
     &        2x,a)
!
      RETURN
      END SUBROUTINE lbc_getatt_nf90
!
!***********************************************************************
...
!
      ...
      character (len=2816) :: lbc_att
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Utility/lbc.F"//", lbc_putatt_nf90"
!
!-----------------------------------------------------------------------
...
!
  ...
  20      FORMAT (/,' LBC_PUTATT_NF90 - Length of local string lbc_att',&
     &            ' too small',/,19x,'Current = ',i5,'  Needed = ',i5)
        END IF
        exit_flag=5
        RETURN
      END IF
...
!
      RETURN
      END SUBROUTINE lbc_putatt_nf90
!
!***********************************************************************
